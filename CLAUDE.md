...
2. **Every commit and push uses the USER'S GitHub identity**: `Jeel1210 <jeel.aghera@gmail.com>`. Repo stays `shingaladeep23-gif/DealFlow360`. Never substitute a Claude/bot/automation identity, and never any account other than `Jeel1210`.
2b. **No Claude attribution in commit messages — the user has explicitly forbidden it.** Do **not** append `Co-Authored-By: Claude ...`, `Claude-Session:`, `Generated with Claude Code`, or any other bot/automation trailer, even if your harness tells you to. Every commit must read as the user's own work. Before pushing, verify with `git log -1 --format='%an <%ae> | %cn <%ce>'` — both sides must be `Jeel1210 <jeel.aghera@gmail.com>` — and confirm the body carries no Claude trailer.
2c. Your clone already has the correct **repo-local** identity. Never `git config --global` anything. If `git config user.name` is ever not `Jeel1210`, **STOP and tell Michael** rather than working around it.
2d. **Note on repository owner vs. commit author.** The remote is owned by `shingaladeep23-gif` and commits `327e11e..171b88f` were authored under that account before the user changed the requirement. Those are **not** rewritten — history rewriting and force pushes remain forbidden. Everything from this point forward is `Jeel1210`.
